...
import { Megaphone, Save, Loader2, Check, Info, Wrench, AlertTriangle, Tag, X } from 'lucide-react';
...
const BannerSettings = () => {
    ...
    const handleSave = async () => {
        if (draft.enabled && !draft.textCs.trim() && !draft.textEn.trim()) {
    ...
    return (
        <div className="max-w-3xl mx-auto pb-32">
            ...
            {/* Zapnout / vypnout */}
            <div className={`rounded-3xl border-2 p-5 sm:p-6 mb-5 transition-all ${draft.enabled ? 'border-primary/40 bg-primary/5' : 'border-olive/10 bg-white'}`}>
                <div className="flex items-center justify-between gap-4">
                    <div>
                        <div className="font-black text-olive-dark uppercase tracking-tight">Zobrazit banner na webu</div>
                        <div className="text-[11px] text-olive/50 uppercase tracking-widest">{draft.enabled ? 'Zapnuto' : 'Vypnuto'}</div>
                    </div>
                    <button type="button" onClick={() => update({ enabled: !draft.enabled })}
                        className={`w-12 h-7 rounded-full flex items-center px-1 transition-all ${draft.enabled ? 'bg-primary justify-end' : 'bg-olive/20 justify-start'}`} aria-pressed={draft.enabled}>
