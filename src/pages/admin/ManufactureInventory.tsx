    ...
    return (
        <div className="space-y-10 pb-20 animate-in fade-in duration-700">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 flex-wrap">
                <div>
                    <h2 className="text-4xl font-black tracking-tight text-olive-dark font-display uppercase italic">Sklad výroby</h2>
                    <p className="text-brand-muted font-bold text-xs uppercase tracking-[0.3em] mt-1">Suroviny a materiály pro produkci</p>
                </div>
                <Button onClick={() => setIsAddOpen(true)} className="h-14 px-8 bg-olive-dark hover:bg-olive text-lime gap-3 font-black uppercase text-xs tracking-[0.2em] rounded-2xl shadow-xl transition-transform hover:scale-[1.02] active:scale-[0.98]">
                                ...
                                <TableRow key={m.id}>
                                    <TableCell>
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 bg-olive-dark/10 rounded-lg flex items-center justify-center text-olive relative">
                                                <Beaker className="w-4 h-4" />
                                                {m.notifications_enabled && (
                                                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-lime rounded-full border-2 border-cream flex items-center justify-center">
                                                        <Bell className="w-1.5 h-1.5 text-olive-dark" />
                                                    </div>
                                                )}
                                            </div>
                                            <div>
                                        ...
                                        {(() => {
                                            ...
                                            return (
                                                <>
                                                    ...
                                                    {isCritical && (
                                                        <p className="text-[10px] text-terracotta font-bold uppercase">Kritický stav!</p>
                                                    )}
                                                    {isWarning && (
                                                        <p className="text-[10px] text-orange font-bold uppercase">Dochází...</p>
                                                    )}
                                                </>
                                            );
                                        })()}
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex justify-end gap-2">
                                            ...
                                            <Button
                                                size="sm"
                                                className="bg-olive-dark hover:bg-olive text-lime"
