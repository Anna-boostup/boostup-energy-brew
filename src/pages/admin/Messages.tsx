                                    ...
                                    <button
                                        ...
                                        className={`w-full text-left p-6 rounded-[2rem] transition-all duration-500 relative group border-2 ${
                                            selectedMessageId === msg.id 
                                                ...
                                                : "bg-transparent border-transparent hover:bg-white/50 hover:border-background"
                                        }`}
                                    >
                                        {!msg.is_read && (
                                        ...
                                        <div className="flex flex-col gap-3">
                                            <div className="flex justify-between items-start">
                                                <span className={`text-[10px] uppercase font-black tracking-[0.15em] ${msg.is_read ? 'text-olive/40' : 'text-primary'}`}>
                                                    {format(new Date(msg.created_at), "d. MMM · HH:mm", { locale: cs })}
                                                </span>
                                            </div>
                                            <div className="space-y-1">
                                                <h3 className={`text-sm font-black uppercase tracking-tight line-clamp-1 ${selectedMessageId === msg.id ? 'text-olive-dark' : 'text-olive'}`}>
                                                    {msg.subject}
                                                </h3>
                                                <p className={`text-[11px] font-bold ${selectedMessageId === msg.id ? 'text-olive/50' : 'text-olive/40'} uppercase tracking-widest`}>
                                                    {msg.from_name || msg.from_email.split('@')[0]}
                                                </p>
                                            </div>
                                            <p className="text-xs text-olive/40 line-clamp-2 leading-relaxed font-bold italic opacity-80">
                                                "{msg.body_text}"
                                            </p>
                                            {msg.replied_at && (
                        ...
                        {selectedMessage ? (
                                        ...
                                        <div className="flex gap-3 w-full sm:w-auto">
                                            ...
                                            <AlertDialog>
                                                <AlertDialogTrigger asChild>
                                                    <Button variant="ghost" size="icon" className="h-14 w-14 rounded-2xl bg-white border border-background text-olive/20 hover:text-red-500 hover:bg-red-50 transition-all">
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center p-20 text-center">
                                <motion.div 
                                    ...
                                    className="p-16 bg-white/40 rounded-[3rem] border-2 border-dashed border-background shadow-inner group"
                                >
