...
-- Nastavení tajného klíče pro šifrování
-- Můžete spustit v SQL editoru: ALTER DATABASE postgres SET "app.settings.mailbox_key" TO 'super-tajne-heslo';
ALTER DATABASE postgres SET "app.settings.mailbox_key" TO 'boostup-initial-secret-key-123';

-- Ujištění, že nová nastavení platí pro současnou session
SELECT set_config('app.settings.mailbox_key', 'boostup-initial-secret-key-123', false);
...
BEGIN
    ...
    IF NEW.imap_password IS NOT NULL 
       ...
       AND (OLD.imap_password IS NULL OR NEW.imap_password != OLD.imap_password) THEN
        NEW.imap_password := encode(encrypt(NEW.imap_password::bytea, current_setting('app.settings.mailbox_key'), 'aes'), 'hex');
    ...
    IF NEW.smtp_password IS NOT NULL 
       ...
       AND (OLD.smtp_password IS NULL OR NEW.smtp_password != OLD.smtp_password) THEN
        NEW.smtp_password := encode(encrypt(NEW.smtp_password::bytea, current_setting('app.settings.mailbox_key'), 'aes'), 'hex');
...
BEGIN
    ...
    SELECT 
        ...
        m.imap_port,
        m.imap_user,
        convert_from(decrypt(decode(m.imap_password, 'hex'), current_setting('app.settings.mailbox_key'), 'aes'), 'utf8') AS imap_password,
        m.smtp_host,
        m.smtp_port,
        m.smtp_user,
        convert_from(decrypt(decode(m.smtp_password, 'hex'), current_setting('app.settings.mailbox_key'), 'aes'), 'utf8') AS smtp_password,
        m.is_active
